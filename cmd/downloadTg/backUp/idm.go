...
func main() {
	...
	err = client.Run(context.Background(), func(ctx context.Context) error {
		...
		http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
			...
			// 这里的 ctx 是请求的 context，如果 IDM 断开连接会取消下载
			reqCtx := r.Context()

			fmt.Println("🚀 开始向 IDM 传输数据 (后端 32 线程拉取中)...")
			startTime := time.Now()

			// 使用 32 线程并发从 Telegram 下载，并流式传输给 HTTP 响应
			// 注意：这里不支持 HTTP Range (断点续传)，IDM 只能单线程连接此代理，
			// 但代理本身会用 32 线程去拉取 Telegram 数据，所以速度依然很快。
			if _, err := d.Download(api, media).WithThreads(1).Stream(reqCtx, w); err != nil {
				fmt.Printf("❌ 传输中断: %v\n", err)
				return
			}
			fmt.Printf("✨ 传输完成! 耗时: %v\n", time.Since(startTime))
		})

		localURL := fmt.Sprintf("http://127.0.0.1:%s/%s", port, fileName)
		fmt.Println("\n🚀 本地中转服务已启动！")
		fmt.Printf("👉 请将此链接复制到 IDM 下载: %s\n", localURL)
		fmt.Println("⏳ 等待连接中... (下载完成后请按 Ctrl+C 停止程序)")
