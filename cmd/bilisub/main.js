...
!function() {
    ...
    function copy_subtitle_text() {
    var p = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : 0;
    api.get_subtitle_data(p, function (vttData) {
        if (!vttData) {
            message_Message_warning("未发现字幕");
            return;
        }

        var lines = vttData.split("\n");
        var textLines = [];
        // 匹配 VTT 时间戳行，如 00:00:01.234 --> 00:00:03.567
        var timestampRegex = /^\d{2}:\d{2}:\d{2}\.\d{3}\s*-->/;

        for (var i = 0; i < lines.length; i++) {
            var line = lines[i].trim();
            // 跳过：空行、WEBVTT头部、时间戳行、纯数字序号行
            if (
                !line ||
                line === "WEBVTT" ||
        ...
        if (!text) {
            message_Message_warning("字幕内容为空");
            return;
        }
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(text).then(function () {
                message_Message_success("字幕已复制到剪切板");
            }).catch(function () {
                fallbackCopy(text);
            });
        } else {
